...
import { RouterHelper } from "@/src/utils/routes";
      ...
      <main className="flex-1 flex flex-col min-h-screen px-3 md:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-7 mb-7">
          ...
          <GlassCard>
            ...
            <div className="text-white/90 text-sm space-y-1">
              <div>
                <span className="font-semibold">Env:</span> {org.sdkConfig.env}
              </div>
              <div>
                <span className="font-semibold">Version:</span>{" "}
                {org.sdkConfig.version}
              </div>
              <div>
                <span className="font-semibold">Tracking:</span>{" "}
                {org.sdkConfig.tracking ? "On" : "Off"}
              </div>
              <div>
                <span className="font-semibold">Domains:</span>{" "}
                {org.sdkConfig.domains.join(", ")}
              </div>
            </div>
        ...
        <div className="mt-7 grid grid-cols-1 sm:grid-cols-3 gap-4">
          <button
            onClick={() => router.push(RouterHelper.Sdkconfig)}
            className="flex flex-col items-center justify-center gap-2 p-4 bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-lg hover:bg-white/10 transition"
          >
            <FileEdit className="text-blue-400 w-8 h-8" />
            <span className="text-white font-medium">Edit SDK Config</span>
          </button>
          <button
            onClick={() => router.push(RouterHelper.ConsentLogs)}
            className="flex flex-col items-center justify-center gap-2 p-4 bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-lg hover:bg-white/10 transition"
          >
          ...
          <button
            onClick={() => router.push(RouterHelper.ApiKeys)}
            className="flex flex-col items-center justify-center gap-2 p-4 bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-lg hover:bg-white/10 transition"
          >
